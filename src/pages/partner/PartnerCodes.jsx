    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-0 md:p-4" style={{backgroundColor:'rgba(26,43,74,0.5)'}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="bg-white w-full h-full md:h-auto md:rounded-3xl md:max-w-xl md:max-h-[90vh] overflow-y-auto" style={{boxShadow:'0 20px 60px rgba(43,191,179,0.15)'}}>
        <div className="flex items-center justify-between p-5 border-b sticky top-0 bg-white z-10" style={{borderColor:'#f4f5f7'}}>
          <div><h2 className="text-lg font-bold" style={{color:'#1a2b4a'}}>{step===1&&'Importer des salariés'}{step===2&&'Mapper les colonnes'}{step===3&&'Import en cours...'}{step===4&&'Terminé'}</h2></div>
...
function AddBeneficiaryModal({ onClose, onAdd, partnerId }) {
  ...
  const handleSubmit = async e => {
    ...
    try { const created = await xano.create('beneficiaries', { ...form, partner_id: partnerId, code:'', status:'pending' }); onAdd(created); onClose() }
...
export default function PartnerCodes({ partnerId }) {
  ...
  return (
    <div>
      {showAddModal && <AddBeneficiaryModal onClose={() => setShowAddModal(false)} onAdd={c => setBeneficiaries(p => [...p, c])} partnerId={partnerId} />}
      {showImportModal && <ImportModal onClose={() => setShowImportModal(false)} onImport={i => setBeneficiaries(p => [...p, ...i])} partnerId={partnerId} />}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        {[{ l:'Disponibles',v:availableCodes.length,c:'#2BBFB3' },{ l:'Envoyés',v:sentCount,c:'#1a2b4a' },{ l:'Salariés',v:beneficiaries.length,c:'#d97706' },{ l:'Utilisation',v:`${usageRate}%`,c:usageRate>80?'#ef4444':'#8a93a2' }].map(s => (
          <div key={s.l} className="bg-white rounded-2xl p-4" style={{ boxShadow: '0 4px 24px rgba(43,191,179,0.06)' }}>
            <p className="text-xl font-bold mb-1" style={{ color: s.c }}>{s.v}</p><p className="text-xs" style={{ color: '#8a93a2' }}>{s.l}</p>
          </div>
        ))}
      </div>
      ...
      {toast && <div className="rounded-2xl p-3 mb-4 flex items-center gap-3" style={{ backgroundColor: '#e8f8f7', border: '1px solid #2BBFB3' }}><div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0" style={{ backgroundColor: '#2BBFB3' }}>✓</div><p className="text-sm" style={{ color: '#1a2b4a' }}>Code <strong style={{ fontFamily: 'monospace' }}>{toast.code}</strong> envoyé à <strong>{toast.name}</strong></p><button onClick={() => setToast(null)} className="ml-auto" style={{ color: '#8a93a2' }}>×</button></div>}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        ...
        {tab === 'beneficiaries' && <div className="flex gap-2"><button onClick={() => setShowImportModal(true)} className="px-4 py-2.5 rounded-xl text-sm font-semibold" style={{ backgroundColor: '#f4f5f7', color: '#1a2b4a' }}>📄 CSV</button><button onClick={() => setShowAddModal(true)} className="px-4 py-2.5 rounded-xl text-white text-sm font-semibold" style={{ backgroundColor: '#2BBFB3' }}>+ Ajouter</button></div>}
      </div>

      {tab === 'beneficiaries' && <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher..." className="w-full px-4 py-3 rounded-2xl text-sm outline-none mb-4" style={{ backgroundColor: '#f4f5f7', color: '#1a2b4a' }} />}

      {tab === 'beneficiaries' && (
        filteredBeneficiaries.length === 0 ? (
          <div className="bg-white rounded-3xl p-10 text-center" style={{ boxShadow: '0 4px 24px rgba(43,191,179,0.06)' }}><span className="text-4xl block mb-3">👥</span><p className="font-bold" style={{ color: '#1a2b4a' }}>{search ? 'Aucun résultat' : 'Aucun salarié'}</p></div>
        ) : (
          <div className="flex flex-col gap-2">{filteredBeneficiaries.map(b => {
            ...
            return (
              <div key={b.id} className="bg-white rounded-2xl px-4 py-4" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.04)', border: isSending ? '1.5px solid #2BBFB3' : '1.5px solid transparent', opacity: hasSent ? 0.75 : 1 }}>
                ...
                {hasSent && b.code && <div className="flex items-center gap-2 mt-2 pt-2" style={{ borderTop: '1px solid #f4f5f7' }}><div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: '#2BBFB3' }} /><p className="text-xs" style={{ color: '#8a93a2' }}>Code <span style={{ fontFamily: 'monospace', color: '#1a2b4a' }}>{b.code}</span>{b.sent_at && ` · ${new Date(b.sent_at).toLocaleDateString('fr-FR')}`}</p></div>}
                {isSending && !hasSent && (
      ...
      {tab === 'codes' && (
        codes.length === 0 ? (
          <div className="bg-white rounded-3xl p-12 text-center" style={{ boxShadow: '0 4px 24px rgba(43,191,179,0.06)' }}><span className="text-4xl">🔑</span><p className="font-semibold mt-4" style={{ color: '#1a2b4a' }}>Aucun code</p></div>
        ) : (
          <div className="bg-white rounded-2xl overflow-hidden" style={{ boxShadow: '0 4px 24px rgba(43,191,179,0.06)' }}>
            <div className="grid grid-cols-3 px-4 py-3 text-xs font-semibold" style={{ backgroundColor: '#f4f5f7', color: '#8a93a2' }}><span>CODE</span><span>STATUT</span><span>ATTRIBUÉ À</span></div>
            {codes.map((c, i) => {
              const assignedTo = beneficiaries.find(b => b.code === c.code)
              return (
                <div key={c.id} className="grid grid-cols-3 px-4 py-3 items-center" style={{ borderTop: i > 0 ? '0.5px solid #f4f5f7' : 'none' }}>
                  ...
                  <span className="text-xs px-2 py-1 rounded-lg font-medium w-fit" style={{ backgroundColor: c.used ? '#fee2e2' : (assignedTo ? '#e8f0fe' : '#e8f8f7'), color: c.used ? '#ef4444' : (assignedTo ? '#1a2b4a' : '#2BBFB3') }}>{c.used ? 'Utilisé' : (assignedTo ? 'Envoyé' : 'Disponible')}</span>
