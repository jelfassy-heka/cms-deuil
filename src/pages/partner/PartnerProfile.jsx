      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-2xl p-5 md:p-6" style={{ boxShadow: '0 4px 24px rgba(43,191,179,0.06)' }}>
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white text-base font-bold flex-shrink-0" style={{ backgroundColor: '#2BBFB3' }}>{partnerInfo?.name?.[0] || '?'}</div>
            <h2 className="font-bold text-base" style={{ color: '#1a2b4a' }}>Informations de l'entreprise</h2>
          </div>
          {isAdmin ? (
            <form onSubmit={handleSaveInfo}>
              {[{key:'name',label:'Nom de l\'entreprise',required:true},{key:'email_contact',label:'Email de contact',type:'email',required:true},{key:'phone',label:'Téléphone',type:'tel'}].map(f=>(
                <div key={f.key} className="mb-4">
                  ...
                  <input type={f.type||'text'} value={form[f.key]} onChange={e=>setForm({...form,[f.key]:e.target.value})} className="w-full px-4 py-3 rounded-2xl text-sm outline-none" style={{backgroundColor:'#f4f5f7',color:'#1a2b4a'}} />
                </div>
              ))}
              <div className="mb-4"><label className="block text-sm font-semibold mb-1.5" style={{color:'#1a2b4a'}}>Type</label><select value={form.partner_type} onChange={e=>setForm({...form,partner_type:e.target.value})} className="w-full px-4 py-3 rounded-2xl text-sm outline-none" style={{backgroundColor:'#f4f5f7',color:'#1a2b4a'}}><option value="entreprise">Entreprise</option><option value="mutuelle">Mutuelle</option><option value="association">Association</option><option value="collectivité">Collectivité</option></select></div>
              <p className="text-xs font-semibold mb-3 mt-6" style={{color:'#8a93a2'}}>CONTACT PRINCIPAL</p>
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div><label className="block text-xs font-semibold mb-1" style={{color:'#1a2b4a'}}>Prénom</label><input value={form.contact_firstname} onChange={e=>setForm({...form,contact_firstname:e.target.value})} className="w-full px-3 py-2.5 rounded-xl text-sm outline-none" style={{backgroundColor:'#f4f5f7',color:'#1a2b4a'}} /></div>
                <div><label className="block text-xs font-semibold mb-1" style={{color:'#1a2b4a'}}>Nom</label><input value={form.contact_lastname} onChange={e=>setForm({...form,contact_lastname:e.target.value})} className="w-full px-3 py-2.5 rounded-xl text-sm outline-none" style={{backgroundColor:'#f4f5f7',color:'#1a2b4a'}} /></div>
              </div>
              <div className="mb-4"><label className="block text-xs font-semibold mb-1" style={{color:'#1a2b4a'}}>Fonction</label><input value={form.contact_role} onChange={e=>setForm({...form,contact_role:e.target.value})} className="w-full px-3 py-2.5 rounded-xl text-sm outline-none" style={{backgroundColor:'#f4f5f7',color:'#1a2b4a'}} /></div>
              <button type="submit" disabled={saving} className="w-full py-3 rounded-2xl text-white text-sm font-semibold" style={{backgroundColor:saving?'#8a93a2':'#2BBFB3'}}>{saving?'Enregistrement...':'Enregistrer les modifications'}</button>
            </form>
          ) : (
            <div>
              {[
                { label: 'Nom de l\'entreprise', value: form.name },
                { label: 'Email de contact', value: form.email_contact },
                { label: 'Téléphone', value: form.phone || '—' },
                { label: 'Type', value: form.partner_type },
              ].map(item => (
                <div key={item.label} className="mb-4">
                  <p className="text-xs font-semibold mb-1" style={{color:'#8a93a2'}}>{item.label.toUpperCase()}</p>
                  <p className="text-sm" style={{color:'#1a2b4a'}}>{item.value || '—'}</p>
                </div>
              ))}
              <p className="text-xs font-semibold mb-3 mt-6" style={{color:'#8a93a2'}}>CONTACT PRINCIPAL</p>
              <div className="mb-3">
                <p className="text-sm" style={{color:'#1a2b4a'}}>
                  {[form.contact_firstname, form.contact_lastname].filter(Boolean).join(' ') || '—'}
                  {form.contact_role && <span style={{color:'#8a93a2'}}> · {form.contact_role}</span>}
                </p>
              </div>
              <div className="rounded-xl p-3 mt-4" style={{backgroundColor:'#f4f5f7'}}>
                <p className="text-xs" style={{color:'#8a93a2'}}>Seuls les administrateurs peuvent modifier ces informations.</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div>
          <div className="bg-white rounded-2xl p-5 md:p-6" style={{boxShadow:'0 4px 24px rgba(43,191,179,0.06)'}}>
            <h2 className="font-bold text-base mb-1" style={{color:'#1a2b4a'}}>Mon compte</h2>
            <p className="text-xs mb-4" style={{color:'#8a93a2'}}>Connecté en tant que {user.email} · {isAdmin ? 'Administrateur' : 'Membre'}</p>

            <p className="text-xs font-semibold mb-3 mt-2" style={{color:'#8a93a2'}}>CHANGER LE MOT DE PASSE</p>
