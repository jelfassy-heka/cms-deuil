#!/bin/bash
# Batch 7D.2.e — Tests audit DELETE /code_request/{id}
# Workspace 17, group api:M9mahf09.
#
# Couvre :
#  - 401 sans bearer / bearer invalide
#  - 404 id inexistant
#  - 200 admin DELETE → audit action_type='delete' créé
#  - 404 ré-DELETE même id → aucun audit supplémentaire
#
# Ne touche pas code_request id=1 et id=2 (rows production préexistantes).
# Aucun bearer affiché en clair.
# Usage : bash scripts/batch7d-tests-1332-coderequest-delete.sh
...
TS=$(date +%s)
TEST_REASON="BATCH7D_DELETE_TEST_${TS}"
TEST_MESSAGE="BATCH7D delete message should not appear in audit"
...
T1=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE "$B/code_request/$ROW_ID")
hcheck "T1 sans bearer" "401" "$T1" ""

T2=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE -H "Authorization: Bearer FAKE_INVALID" "$B/code_request/$ROW_ID")
hcheck "T2 bearer invalide" "401" "$T2" ""

echo ""
echo "=== Volet B : id inexistant (aucun audit attendu) ==="
T3=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE -H "Authorization: Bearer $ADMIN_TOKEN" "$B/code_request/999999")
hcheck "T3 id inexistant" "404" "$T3" ""

echo ""
echo "=== Volet C : DELETE admin réussi (audit delete attendu) ==="
...
hcheck "T4 admin DELETE code_request id=$ROW_ID" "200" "$T4_CODE" ""
echo "    audit attendu : action_type='delete', action_label='Deleted code request',"
echo "                    previous_values whitelist 11 champs SANS message,"
echo "                    metadata.message_present=true"

echo ""
echo "=== Vérif post-DELETE : ré-DELETE même id → 404 (preuve disparition, aucun audit supplémentaire) ==="
T5=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE -H "Authorization: Bearer $ADMIN_TOKEN" "$B/code_request/$ROW_ID")
hcheck "T5 ré-DELETE même id" "404" "$T5" ""

echo ""
echo "=== Fin tests Batch 7D.2.e ==="
echo ""
echo ">>> code_request id=$ROW_ID auto-supprimé par T4."
echo ">>> audit_logs à supprimer côté Claude (object_type='code_request' object_id=$ROW_ID) :"
echo "    - 1 audit create (setup 1330)"
echo "    - 1 audit delete (T4) ← cible"
echo "    + 1 audit login admin (script)"
echo ""
echo ">>> Le contenu du message NE DOIT JAMAIS apparaître dans l'audit delete."
