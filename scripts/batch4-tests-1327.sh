#!/bin/bash
# Batch 4 — Test endpoint #12 : 1327 PATCH /code_request/{id}
# Workspace 17, group api:M9mahf09. Admin-only + 404 + PATCH partiel attendus.
#
# Le script :
#   1) crée une demande test DÉDIÉE via 1330 POST /code_request (sécurisé Batch 3A)
...
#
# Usage :
#   PARTNER_EMAIL='partenaire@...' bash scripts/batch4-tests-1327.sh
...
ORIG_REASON=$(echo "$A_BODY" | python3 -c "import json,sys;print(json.load(sys.stdin).get('reason',''))" 2>/dev/null)
ORIG_QUANTITY=$(echo "$A_BODY" | python3 -c "import json,sys;print(json.load(sys.stdin).get('quantity',''))" 2>/dev/null)

# ─── Garde-fou : refuser de continuer si ROW_ID ∈ {1,2} (rows pré-existantes)
if [ "$ROW_ID" = "1" ] || [ "$ROW_ID" = "2" ]; then
  echo "ANOMALIE : la demande test a id=$ROW_ID (row pré-existante). Stop."
  exit 1
fi

echo "Demande test créée — id=$ROW_ID, reason='$ORIG_REASON', quantity=$ORIG_QUANTITY, request_status='pending'"
echo ""

# ─── T1 : PATCH sans bearer → 401
echo "=== T1 PATCH /code_request/$ROW_ID sans bearer ==="
T1_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X PATCH -H 'Content-Type: application/json' -d '{"request_status":"T1_NOSEC"}' "$B/code_request/$ROW_ID")
echo "[HTTP $T1_CODE] (attendu 401)"
echo ""

# ─── T2 : PATCH bearer partenaire → 403
if [ -n "$PARTNER_TOKEN" ]; then
  echo "=== T2 PATCH /code_request/$ROW_ID bearer partenaire ==="
  T2_RESP=$(curl -s -w "\n%{http_code}" -X PATCH -H "Authorization: Bearer $PARTNER_TOKEN" -H 'Content-Type: application/json' -d '{"request_status":"T2_PARTNER"}' "$B/code_request/$ROW_ID")
...
# ─── T3 : PATCH bearer admin id existant — UN SEUL champ envoyé (request_status)
echo "=== T3 PATCH /code_request/$ROW_ID bearer admin (un seul champ envoyé : request_status) ==="
T3_RESP=$(curl -s -w "\n%{http_code}" -X PATCH -H "Authorization: Bearer $ADMIN_TOKEN" -H 'Content-Type: application/json' -d '{"request_status":"processing"}' "$B/code_request/$ROW_ID")
...
echo "=== Vérification PATCH partiel (request_status changé, autres préservés) ==="
echo "$T3_BODY" | python3 -c "
import json, sys
orig_reason = '$ORIG_REASON'
orig_quantity = $ORIG_QUANTITY if '$ORIG_QUANTITY'.isdigit() else None
try:
    ...
    print(f'  request_status après PATCH = {rs!r} (attendu \"processing\")')
    print(f'  reason après PATCH         = {rea!r} (attendu {orig_reason!r}, inchangé)')
    print(f'  quantity après PATCH       = {qty} (attendu {orig_quantity}, inchangé)')
    print(f'  request_type après PATCH   = {rt!r} (attendu \"codes\", inchangé)')
    print(f'  partner_id après PATCH     = {pid} (attendu 1, inchangé)')
    ok = (rs == 'processing' and rea == orig_reason and qty == orig_quantity and rt == 'codes')
    print('')
    if ok:
        print('  >>> PATCH partiel OK (champs non envoyés préservés)')
    else:
        print('  >>> ANOMALIE : champs non envoyés modifiés ou request_status non changé')
...
echo "=== T4 PATCH /code_request/$FAKE_ID bearer admin (id inexistant) ==="
T4_RESP=$(curl -s -w "\n%{http_code}" -X PATCH -H "Authorization: Bearer $ADMIN_TOKEN" -H 'Content-Type: application/json' -d '{"request_status":"T4_NOTFOUND"}' "$B/code_request/$FAKE_ID")
...
# ─── Restauration request_status='pending'
echo "=== Restauration request_status='pending' ==="
RESTORE_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X PATCH -H "Authorization: Bearer $ADMIN_TOKEN" -H 'Content-Type: application/json' -d '{"request_status":"pending"}' "$B/code_request/$ROW_ID")
