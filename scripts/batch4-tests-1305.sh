#!/bin/bash
# Batch 4 — Test endpoint #5 : 1305 DELETE /crm_activity/{id}
# Workspace 17, group api:M9mahf09. Admin-only + 404 attendus.
#
# Le script :
#   1) crée une row test via 1308 POST /crm_activity (note="ORIGINAL_1305")
#   2) teste DELETE (sans bearer / partenaire / admin id inexistant)
#      sur des cibles qui ne suppriment PAS la row test
#   3) teste DELETE admin sur la row test → suppression effective
#   4) vérifie la disparition (GET 1306 → 404)
#
...
#
# Usage :
#   PARTNER_EMAIL='partenaire@...' bash scripts/batch4-tests-1305.sh
...
A_PAYLOAD=$(cat <<EOF
{
  ...
  "activity_type": "BATCH4_TEST_1305",
  "note": "ORIGINAL_1305 — sera supprimée par T4",
  "crm_status": "test",
...
echo "=== T1 DELETE /crm_activity/$ROW_ID sans bearer ==="
T1_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE "$B/crm_activity/$ROW_ID")
echo "[HTTP $T1_CODE] (attendu 401)"
echo ""

# ─── T2 : DELETE bearer partenaire sur ROW_ID → 403 (la row reste)
if [ -n "$PARTNER_TOKEN" ]; then
  echo "=== T2 DELETE /crm_activity/$ROW_ID bearer partenaire ==="
  T2_RESP=$(curl -s -w "\n%{http_code}" -X DELETE -H "Authorization: Bearer $PARTNER_TOKEN" "$B/crm_activity/$ROW_ID")
...
echo "=== T3 DELETE /crm_activity/$FAKE_ID bearer admin (id inexistant) ==="
T3_RESP=$(curl -s -w "\n%{http_code}" -X DELETE -H "Authorization: Bearer $ADMIN_TOKEN" "$B/crm_activity/$FAKE_ID")
...
# ─── Vérification que ROW_ID existe toujours après T1+T2+T3 (via GET 1306 sécurisé)
echo "=== Vérif row test toujours présente avant T4 (via GET admin) ==="
PRE_T4_CODE=$(curl -s -o /dev/null -w "%{http_code}" -H "Authorization: Bearer $ADMIN_TOKEN" "$B/crm_activity/$ROW_ID")
echo "[HTTP $PRE_T4_CODE] (attendu 200)"
echo ""

# ─── T4 : DELETE bearer admin sur ROW_ID → 200, suppression effective
echo "=== T4 DELETE /crm_activity/$ROW_ID bearer admin (suppression effective) ==="
...
# ─── Vérification post-T4 : la row a bien disparu (GET → 404)
echo "=== Vérif post-T4 : GET /crm_activity/$ROW_ID admin doit renvoyer 404 ==="
POST_T4_CODE=$(curl -s -o /dev/null -w "%{http_code}" -H "Authorization: Bearer $ADMIN_TOKEN" "$B/crm_activity/$ROW_ID")
